...
async function canManageBankTransactions ({ authentication: { item: user }, originalInput, operation, itemId, itemIds, listKey }) {
    if (!user) return throwAuthenticationError()
    if (user.deletedAt) return false
    ...
    return await checkPermissionsInUserOrganizationsOrRelatedOrganizations(user.id, organizationIds, 'canManageBankTransactions')
}
